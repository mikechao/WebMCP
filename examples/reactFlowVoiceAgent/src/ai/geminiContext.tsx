...
export const GeminiProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    liveClient.on('interrupted', stopAudioStreamer).on('audio', onAudio).on('toolcall', callTool);
  ...
  const liveConnect = useCallback(
    async (mcpTools?: MCPTool[]): Promise<boolean> => {
  ...
  const mcpConnect = useCallback(
    async (serverType: string): Promise<boolean> => {
      try {
        await mcpDisconnect();
