...
export function createMcpServer(): McpServer {
  ...
  server.tool(
    ...
    async ({ username }) => {
      ...
      const reposData = await makeGitHubRequest(
  ...
  server.tool(
    ...
    async ({ owner, repo, state = 'open' }) => {
      ...
      const issuesData = await makeGitHubRequest(
  ...
  server.tool(
    ...
    async ({ query }) => {
      ...
      const searchData = await makeGitHubRequest(
      ...
      const reposList = searchData.items
        .map(
          (repo: any) =>
