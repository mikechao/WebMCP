...
export function createMcpServer(): McpServer {
  ...
  server.tool(
    'get_current_weather',
    {
      location: z
        .string()
        .describe('US city and state for current weather conditions, e.g. "San Francisco, CA"'),
    },
    async ({ location }) => {
  ...
  server.tool(
    'get_forecast',
    {
      location: z
        .string()
        .describe(
          'US city and state for multi-day weather forecast, e.g. "Plainfield, IL" or "San Francisco, CA"'
        ),
    },
    async ({ location }) => {
  ...
  server.tool(
    'get_alerts',
    {
      state: z
        .string()
        .length(2)
        .describe('Two-letter US state code for weather alerts (e.g., IL, CA, NY)'),
    },
    async ({ state }) => {
