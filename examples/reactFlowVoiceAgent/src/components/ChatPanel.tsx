...
export default function ChatSection({disconnectNode}: {disconnectNode: () => void}) {
    ...
    const { 
        liveClient, liveConnected, sendMessage,
        setTools, tools, 
        mcpDisconnect, liveConnect, liveDisconnect, setLiveConnected,
        voiceModeEnabled, toggleVoiceMode, setApiKey,
    } = useGemini();   
    ...
    const handleConnectNode = async () => {
        ...
        const message = success ? '🔌 Connected to MCP server' : '❌ Failed to connect to MCP server. Make sure you have set an API key.';
    ...
    return (
        <div className="w-[600px] min-w-[300px] max-w-[600px] h-screen p-6 bg-slate-900 border-l border-slate-700 flex flex-col overflow-hidden">
            ...
            {/* API Key Form - Fixed */}
            <div
                className="mb-6 p-5 bg-slate-800 rounded-xl border border-slate-700"
            >
                <input 
                    type="password" 
                    value={apiKeyInput}
