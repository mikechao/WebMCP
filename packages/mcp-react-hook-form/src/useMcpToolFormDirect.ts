...
import { registerFormAsMcpTool, RegisterFormOptions } from "./registerFormAsMcpTool.js";
...
export function useMcpToolFormDirect<T extends ZodObject<ZodRawShape>>(
  ...
  useEffect(() => {
    ...
    return unregister;
  }, [mcpServer, toolName, form, schema, options?.title, options?.description, options?.onToolCall]);
}