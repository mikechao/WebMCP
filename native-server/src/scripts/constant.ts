...
if (!process.env.DEV_EXTENSION_ID && process.env.NODE_ENV !== 'production') {
  throw new Error('DEV_EXTENSION_ID is not defined in the environment variables.');
}
...
export const DEV_EXTENSION_ID = process.env.DEV_EXTENSION_ID; // Development extension ID from .env
